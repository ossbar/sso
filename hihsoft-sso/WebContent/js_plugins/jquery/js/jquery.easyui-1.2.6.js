...
(function($) {
	...
	function _88(_89) {
		...
		$("span.tree-checkbox", _8b).unbind(".tree").bind("click.tree",
				function(e) {
					_8a.event = e;//add by xjf@20120309,点击树的复选框时，将事件传到options中供调用
					var _8d = $(this).parent();
	...
	function _a9(_aa, _ab, _ac) {
		...
		//modified by xjf@20120301
		var _b1 = _b2(_aa, _ab);
		if (_ad.onCheck.call(_aa, _b1, _ac) === false) return;
		...
		//1147,1148 line in there befor modify
		//add by xjf@20120301
		if ($.isFunction(_ad.onAfterCheck)) {
			_ad.onAfterCheck.call(_aa, _b1, _ac);
		}
		// add end-----------
		function _b0(_b3) {
...
//		}
		...
		//add by xjf@20120301
		var checked = $(_c4).tree("getChecked");
		jQuery.each(checked, function(i, n){
			if (_be(_c4, n.target)) {
				_a9(_c4, n.target, true);
			}
		});	
		//----------add end-----------
	...
	function _db(_dc, _dd, _de) {
		...
		if (ul.length) {
			...
			//add by xjf@2012-01-03
			hit.removeClass("tree-expanded ").addClass("tree-indent")
				.unbind("mouseenter.tree mouseleave.tree")
				.next().removeClass("tree-folder-open tree-collapsed-hover")
				.addClass("tree-file");
			return;
			//-----end-----
	...
	function _105(_106) {
		var _107 = [];
		//---add by xjf@20120301 ---
		var url = ".tree-checkbox1";
		var opt = $.data(_106, "tree").options;
		if (opt.includeHalf !== false) url += ",.tree-checkbox2";
		//---add end---
		// modified by xjf@20120301
		$(_106).find(url).each(function() {
	...
	function _1ff(_200) {
		...
		if (opts.height > 0) {
			cc.height($.boxModel == true
					...
					: opts.height);
			var _203 = _201.length ? _201[0].panel("header").css("height", "")
					.outerHeight() : "auto";
...
(function($) {
	...
	function _2ad(_2ae, _2af, _2b0) {
		...
		function _2b3(_2b4, _2b5) {
			...
			var _2b6 = "expand" + _2b4.substring(0, 1).toUpperCase()
					+ _2b4.substring(1);
			if (!_2b1[_2b6]) {
				_2b1[_2b6] = _2b2(_2b4);
		...
		if (_2af == "east") {
			_2b3("east", {
						resizeC : {
							width : _2b1.center.panel("options").width
									+ _2b1["east"].panel("options").width - 28
						},
						expand : {
							left : cc.width()
									- _2b1["east"].panel("options").width
						},
						expandP : {
							top : _2b1["east"].panel("options").top,
							left : cc.width() - 28,
							width : 28,
							height : _2b1["center"].panel("options").height
						},
						collapse : {
							left : cc.width()
						}
			...
			if (_2af == "west") {
				_2b3("west", {
							...
							expandP : {
								left : 0,
								top : _2b1["west"].panel("options").top,
								width : 28,
								height : _2b1["center"].panel("options").height
							},
							collapse : {
								left : -_2b1["west"].panel("options").width
							}
						});
			} else {
				if (_2af == "north") {
					...
					_2b1.east.panel("resize", {
								top : 28,
								height : hh
							});
					_2b1.west.panel("resize", {
								top : 28,
								height : hh
							});
					if (_2a1(_2b1.expandEast)) {
						_2b1.expandEast.panel("resize", {
									top : 28,
									height : hh
								});
					}
					if (_2a1(_2b1.expandWest)) {
						_2b1.expandWest.panel("resize", {
									top : 28,
									height : hh
								});
					}
					_2b3("north", {
								...
								collapse : {
									top : -_2b1["north"].panel("options").height
								}
							});
				} else {
					if (_2af == "south") {
	...
	function _2b7(_2b8, _2b9) {
		...
		function _2bb(_2bc, _2bd) {
			...
			var _2be = "expand" + _2bc.substring(0, 1).toUpperCase()
					+ _2bc.substring(1);
		...
		if (_2b9 == "east" && _2ba.expandEast) {
			...
			if (_2b9 == "west" && _2ba.expandWest) {
				_2bb("west", {
							collapse : {
								left : -_2ba["west"].panel("options").width
							},
							expand : {
								left : 0
							}
				...
				if (_2b9 == "north" && _2ba.expandNorth) {
					_2bb("north", {
								collapse : {
									top : -_2ba["north"].panel("options").height
								},
								expand : {
									top : 0
								}
...
(function($) {
		...
		function _2cf(menu) {
			menu.addClass("menu").find(">div").each(function() {
				...
				if (item.hasClass("menu-sep")) {
					...
					if (_2d1) {
						$("<div class=\"menu-icon\"></div>").addClass(_2d1)
								.appendTo(item);
					}
					if (item[0].submenu) {
	...
	function _2d3(_2d4, item) {
		...
		item.bind("mousedown.menu", function() {
			...
			if (_2d5) {
				...
				if (left + _2d5.outerWidth() + 5 > $(window).width()
						+ $(document).scrollLeft()) {
					left = item.offset().left - _2d5.outerWidth() + 2;
				}
				var top = item.offset().top - 3;
				if (top + _2d5.outerHeight() > $(window).height()
						+ $(document).scrollTop()) {
	...
	function _2da(_2db, pos) {
		...
		if (pos) {
			opts.left = pos.left;
			opts.top = pos.top;
			if (opts.left + $(_2db).outerWidth() > $(window).width()
					+ $(document).scrollLeft()) {
				opts.left = $(window).width() + $(document).scrollLeft()
						- $(_2db).outerWidth() - 5;
			}
			if (opts.top + $(_2db).outerHeight() > $(window).height()
					+ $(document).scrollTop()) {
	...
	function _2e6(_2e7, _2e8) {
		...
		if (_2e8.iconCls) {
			$("<div class=\"menu-icon\"></div>").addClass(_2e8.iconCls)
					.appendTo(item);
		}
		if (_2e8.id) {
	...
	$.fn.menu = function(_2f0, _2f1) {
		...
		return this.each(function() {
					...
					$(this).css({
								left : _2f2.options.left,
								top : _2f2.options.top
							});
				});
	};
	$.fn.menu.methods = {
		...
		setText : function(jq, _2f3) {
			return jq.each(function() {
						$(_2f3.target).children("div.menu-text")
								.html(_2f3.text);
					});
		},
			...
			return jq.each(function() {
						...
						if (item.iconCls) {
							$(item.target).children("div.menu-icon")
									...
									.addClass(_2f4.iconCls);
						} else {
							$("<div class=\"menu-icon\"></div>")
									.addClass(_2f4.iconCls)
									.appendTo(_2f4.target);
						}
					});
		},
		getItem : function(jq, _2f5) {
			var item = {
				target : _2f5,
				id : $(_2f5).attr("id"),
				text : $.trim($(_2f5).children("div.menu-text").html()),
				disabled : $(_2f5).hasClass("menu-item-disabled"),
				href : $(_2f5).attr("href"),
				onclick : _2f5.onclick
			};
...
(function($) {
	...
	$.fn.menubutton.parseOptions = function(_303) {
		var t = $(_303);
		return $.extend({}, $.fn.linkbutton.parseOptions(_303), {
					menu : t.attr("menu"),
...
(function($) {
	...
	$.fn.splitbutton.parseOptions = function(_30e) {
		var t = $(_30e);
		return $.extend({}, $.fn.linkbutton.parseOptions(_30e), {
					menu : t.attr("menu"),
					duration : t.attr("duration")
				});
	};
...
(function($) {
	function init(_30f) {
		...
		var _310 = $("<input type=\"text\" class=\"searchbox-text\">")
				.appendTo(span);
	...
	function _311(_312) {
		...
		var _313 = opts.width - sb.find("a.searchbox-menu").outerWidth()
	...
	function _314(_315) {
		...
		function _317(item) {
			...
			var mb = $("<a class=\"searchbox-menu\" href=\"javascript:void(0)\"></a>")
					.html(item.text);
			mb.prependTo(_316.searchbox).menubutton({
						menu : _316.menu,
						iconCls : item.iconCls
					});
			_316.searchbox.find("input.searchbox-text").attr("name",
	...
	function _319(_31a) {
		...
		_31c.unbind(".searchbox").bind("blur.searchbox", function(e) {
					...
					if (e.keyCode == 13) {
						e.preventDefault();
						var name = $.fn.prop ? _31c.prop("name") : _31c
								.attr("name");
						opts.value = $(this).val();
	...
	$.fn.searchbox.parseOptions = function(_327) {
		var t = $(_327);
		return {
			width : (parseInt(_327.style.width) || undefined),
				...
				if (!rule["validator"](_336, _33a)) {
					...
					if (_33a) {
						for (var i = 0; i < _33a.length; i++) {
							_33b = _33b.replace(new RegExp("\\{" + i + "\\}",
											"g"), _33a[i]);
						}
					}
...
(function($) {
	function _347(_348, _349) {
		...
		var _34b = $("<iframe id=" + _34a + " name=" + _34a + "></iframe>")
				.attr(
						"src",
...
(function($) {
	...
	$.fn.numberbox.parseOptions = function(_375) {
		var t = $(_375);
		return $.extend({}, $.fn.validatebox.parseOptions(_375), {
					disabled : (t.attr("disabled") ? true : undefined),
...
(function($) {
	...
	function _384(_385) {
		...
		if ($(_385).find(".calendar-menu-month-inner").is(":empty")) {
			...
			var t = $("<table></table>").appendTo($(_385)
			...
			$(_385).find(".calendar-menu-prev,.calendar-menu-next").hover(
					function() {
						$(this).addClass("calendar-menu-hover");
					}, function() {
						$(this).removeClass("calendar-menu-hover");
	...
	function _38a(year, _38b) {
		...
		if (_38f.length < 7) {
			while (_38f.length < 7) {
				...
				_38f.unshift([date.getFullYear(), date.getMonth() + 1,
						date.getDate()]);
			}
		} else {
			var _390 = _38f[0];
	...
	function show(_393) {
		...
		$(_393).find(".calendar-title span").html(opts.months[opts.month - 1]
				+ " " + opts.year);
		...
		var now = new Date();
		var _395 = now.getFullYear() + "," + (now.getMonth() + 1) + ","
				+ now.getDate();
		t.find("td[abbr=\"" + _395 + "\"]").addClass("calendar-today");
		if (opts.current) {
			t.find(".calendar-selected").removeClass("calendar-selected");
			var _396 = opts.current.getFullYear() + ","
		...
		t.find("td").hover(function() {
					...
					opts.current = new Date(_397[0], parseInt(_397[1]) - 1,
							_397[2]);
	...
	$.fn.calendar.parseOptions = function(_39b) {
		var t = $(_39b);
		return {
			width : (parseInt(_39b.style.width) || undefined),
			height : (parseInt(_39b.style.height) || undefined),
			fit : (t.attr("fit") ? t.attr("fit") == "true" : undefined),
			border : (t.attr("border") ? t.attr("border") == "true" : undefined)
		};
	};
...
(function($) {
	...
	$.fn.spinner = function(_3aa, _3ab) {
		...
		return this.each(function() {
					...
					if (_3ad) {
						...
						_3ad = $.data(this, "spinner", {
									options : $.extend({},
											$.fn.spinner.defaults, $.fn.spinner
													.parseOptions(this), _3aa),
									spinner : init(this)
								});
						$(this).removeAttr("disabled");
					}
	...
	$.fn.spinner.parseOptions = function(_3b1) {
		var t = $(_3b1);
		return $.extend({}, $.fn.validatebox.parseOptions(_3b1), {
					width : (parseInt(_3b1.style.width) || undefined),
					value : (t.val() || undefined),
					min : t.attr("min"),
					max : t.attr("max"),
...
(function($) {
	...
	$.fn.numberspinner.parseOptions = function(_3bb) {
		return $.extend({}, $.fn.spinner.parseOptions(_3bb), $.fn.numberbox
						.parseOptions(_3bb), {});
	};
...
(function($) {
	...
	$.fn.timespinner.parseOptions = function(_3d5) {
		var t = $(_3d5);
		return $.extend({}, $.fn.spinner.parseOptions(_3d5), {
...
(function($) {
	...
	function _3dd(_3de) {
		...
		if (opts.height != "auto") {
			var _3e8 = _3e0
					- _3e2.children("div.datagrid-header").outerHeight(true)
					- _3e2.children("div.datagrid-footer").outerHeight(true)
	...
	function _3e9(_3ea) {
		...
		if (mask.length) {
			...
			msg.css({
						left : (_3eb.width() - msg.outerWidth()) / 2,
						top : (_3eb.height() - msg.outerHeight()) / 2
					});
		}
	};
	...
	function _3f7(_3f8, _3f9) {
		...
		var _3fd = $("<div class=\"datagrid-wrap\">"
				...
				+ "</div>" + "</div>").insertAfter(_3f8);
		_3fd.panel({
					doSize : false
				});
		_3fd.panel("panel").addClass("datagrid").bind("_resize",
				function(e, _3fe) {
		...
		return {
			...
			dc : {
				...
				view1 : _402,
				view2 : _403,
				body1 : _402.children("div.datagrid-body")
						.children("div.datagrid-body-inner"),
				body2 : _403.children("div.datagrid-body"),
				footer1 : _402.children("div.datagrid-footer")
						.children("div.datagrid-footer-inner"),
				footer2 : _403.children("div.datagrid-footer")
						.children("div.datagrid-footer-inner")
			}
		};
	};
	...
	function _408(_409) {
		...
		_40a.panel($.extend({}, opts, {
					doSize : false,
					onResize : function(_40b, _40c) {
						_3e9(_409);
						setTimeout(function() {
		...
		var _40d = dc.view1;
		var _40e = dc.view2;
		var _40f = _40d.children("div.datagrid-header")
				.children("div.datagrid-header-inner");
		var _410 = _40e.children("div.datagrid-header")
				.children("div.datagrid-header-inner");
		...
		_40d.find("div.datagrid-footer-inner").css("display",
				opts.showFooter ? "block" : "none");
		_40e.find("div.datagrid-footer-inner").css("display",
				opts.showFooter ? "block" : "none");
		if (opts.toolbar) {
			if (typeof opts.toolbar == "string") {
		...
		if (opts.pagination) {
			...
			_412.pagination({
						...
						onSelectPage : function(_413, _414) {
	...
	function _418(_419) {
		...
		tr.unbind(".datagrid").bind("mouseenter.datagrid", function() {
					...
					if (opts.onRowContextMenu) {
						opts.onRowContextMenu.call(_419, e, _41e,
								data.rows[_41e]);
					}
				});
		...
		tr.find("div.datagrid-cell-check input[type=checkbox]")
				.unbind(".datagrid").bind("click.datagrid", function(e) {
					var _425 = $(this).parent().parent().parent()
	...
	function _429(_42a) {
		...
		_42c.find("td:has(div.datagrid-cell)").unbind(".datagrid").bind(
		...
		_42c.find("input[type=checkbox]").unbind(".datagrid").bind(
			...
			$(this).resizable({
				...
				onStartResize : function(e) {
					...
					dc.view.children("div.datagrid-resize-proxy").css({
								left : e.pageX - $(_42b).offset().left - 1,
								display : "block"
							});
					_42e($(this), false);
				},
	...
	function _434(_435) {
		...
		var _43c = _436.width() - _436.find("table").width()
				- opts.scrollbarSize;
		var rate = _43c / _437;
		if (!opts.showHeader) {
			_43b.hide();
		...
		function _43e(col, _43f) {
			col.width += _43f;
			col.boxWidth += _43f;
			_436.find("td[field=\"" + col.field + "\"] div.datagrid-cell")
					.width(col.boxWidth);
		};
	};
	function _3ff(_440, _441) {
		...
		if (_441) {
			...
			var _443 = dc.view1.children("div.datagrid-header").add(dc.view2
		...
		function fix(_444) {
			...
			var bf = opts.finder.getTr(_440, "", "allbody").add(opts.finder
					.getTr(_440, "", "allfooter"));
	...
	function _446(_447) {
		...
		var _449 = dc.view1.children("div.datagrid-header").add(dc.view2
				.children("div.datagrid-header"));
		...
		if (!opts.remoteSort) {
			...
			if (opt) {
				...
				data.rows.sort(function(r1, r2) {
							return _45d(r1[opts.sortName], r2[opts.sortName])
	...
	function _468(_469) {
		...
		var tr = opts.finder.getTr(_469, "", "allbody")
	...
	function _466(_46d) {
		...
		var tr = opts.finder.getTr(_46d, "", "selected")
	...
	function _427(_471, _472) {
		...
		if (top <= 0) {
			...
			if (top + tr.outerHeight() > _475.height() - 18) {
				_475.scrollTop(_475.scrollTop() + top + tr.outerHeight()
						- _475.height() + 18);
			}
		}
	};
			...
			if (col && col.editor) {
				...
				if (_49b) {
					...
					$.data(cell[0], "datagrid.editor", {
								actions : _49b,
								target : _49b.init(cell.find("td"), _49a),
								field : _498,
								type : _499,
								oldHtml : _49c
							});
	...
	var _4d0 = {
		text : {
			init : function(_4d1, _4d2) {
				var _4d3 = $("<input type=\"text\" class=\"datagrid-editable-input\">")
						.appendTo(_4d1);
				return _4d3;
			},
		...
		textarea : {
			init : function(_4da, _4db) {
				var _4dc = $("<textarea class=\"datagrid-editable-input\"></textarea>")
						.appendTo(_4da);
				return _4dc;
			},
		...
		checkbox : {
			...
			setValue : function(_4e7, _4e8) {
				...
				$.fn.prop ? $(_4e7).prop("checked", _4e9) : $(_4e7).attr(
						"checked", _4e9);
			}
		},
		numberbox : {
			init : function(_4ea, _4eb) {
				var _4ec = $("<input type=\"text\" class=\"datagrid-editable-input\">")
						.appendTo(_4ea);
				_4ec.numberbox(_4eb);
				return _4ec;
			},
		...
		validatebox : {
			init : function(_4f4, _4f5) {
				var _4f6 = $("<input type=\"text\" class=\"datagrid-editable-input\">")
						.appendTo(_4f4);
	...
	$.fn.datagrid.methods = {
		options : function(jq) {
			...
			var opts = $.extend(_519, {
						width : _51a.width,
						height : _51a.height,
						closed : _51a.closed,
						collapsed : _51a.collapsed,
						minimized : _51a.minimized,
						maximized : _51a.maximized
					});
			var _51b = jq.datagrid("getPager");
			if (_51b.length) {
				var _51c = _51b.pagination("options");
				$.extend(opts, {
							pageNumber : _51c.pageNumber,
							pageSize : _51c.pageSize
						});
			}
			return opts;
		},
		...
		reloadFooter : function(jq, _523) {
			return jq.each(function() {
						...
						if (opts.showFooter) {
							opts.view.renderFooter.call(opts.view, this, _525
											.find("div.datagrid-footer-inner"),
									false);
							opts.view.renderFooter.call(opts.view, this, _524
											.find("div.datagrid-footer-inner"),
									true);
							if (opts.view.onAfterRender) {
			...
			return jq.each(function() {
				...
				if (opts.loadMsg) {
					...
					$("<div class=\"datagrid-mask-msg\" style=\"display:block\"></div>")
							.html(opts.loadMsg).appendTo(_526);
					_3e9(this);
				}
			});
		},
	...
	var _53c = {
		render : function(_53d, _53e, _53f) {
			...
			for (var i = 0; i < rows.length; i++) {
				var cls = (i % 2 && opts.striped)
						...
						: "";
				var _542 = opts.rowStyler ? opts.rowStyler.call(_53d, i,
						rows[i]) : "";
				var _543 = _542 ? "style=\"" + _542 + "\"" : "";
				_541.push("<tr datagrid-row-index=\"" + i + "\" " + cls + " "
						+ _543 + ">");
				_541.push(this.renderRow.call(this, _53d, _540, _53f, i,
						rows[i]));
				_541.push("</tr>");
			}
		...
		renderFooter : function(_544, _545, _546) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				_548.push(this.renderRow.call(this, _544, _547, _546, i,
						rows[i]));
				_548.push("</tr>");
			}
		...
		renderRow : function(_549, _54a, _54b, _54c, _54d) {
			...
			for (var i = 0; i < _54a.length; i++) {
				...
				if (col) {
					var _550 = col.styler
							? (col.styler(_54d[_54f], _54d, _54c) || "")
							: "";
					var _551 = col.hidden ? "style=\"display:none;" + _550
							+ "\"" : (_550 ? "style=\"" + _550 + "\"" : "");
		...
		refreshRow : function(_552, _553) {
			var row = {};
			var _554 = $(_552).datagrid("getColumnFields", true).concat($(_552)
		...
		updateRow : function(_555, _556, row) {
			...
			for (var _557 in row) {
				...
				if (col) {
					var _558 = col.styler ? col.styler(rows[_556][_557],
							rows[_556], _556) : "";
					td.attr("style", _558 || "");
					if (col.hidden) {
						td.hide();
					...
					if (col.formatter) {
						cell.html(col.formatter(rows[_556][_557], rows[_556],
								_556));
					} else {
			...
			var _558 = opts.rowStyler ? opts.rowStyler.call(_555, _556,
					rows[_556]) : "";
			tr.attr("style", _558 || "");
		...
		insertRow : function(_559, _55a, row) {
			...
			for (var i = data.rows.length - 1; i >= _55a; i--) {
				opts.finder.getTr(_559, i, "body", 2).attr(
						"datagrid-row-index", i + 1);
				var tr = opts.finder.getTr(_559, i, "body", 1).attr(
						"datagrid-row-index", i + 1);
				if (opts.rownumbers) {
			...
			var tr1 = "<tr datagrid-row-index=\"" + _55a + "\">"
					+ this.renderRow.call(this, _559, _55b, true, _55a, row)
					+ "</tr>";
			var tr2 = "<tr datagrid-row-index=\"" + _55a + "\">"
					+ this.renderRow.call(this, _559, _55c, false, _55a, row)
					+ "</tr>";
			if (_55a >= data.rows.length) {
				if (data.rows.length) {
		...
		deleteRow : function(_55d, _55e) {
			...
			for (var i = _55e + 1; i < data.rows.length; i++) {
				opts.finder.getTr(_55d, i, "body", 2).attr(
						"datagrid-row-index", i - 1);
				var tr1 = opts.finder.getTr(_55d, i, "body", 1).attr(
						"datagrid-row-index", i - 1);
				if (opts.rownumbers) {
		...
		onAfterRender : function(_560) {
			...
			if (opts.showFooter) {
				var _561 = $(_560).datagrid("getPanel")
	...
	$.fn.datagrid.defaults = $.extend({}, $.fn.panel.defaults, {
		...
		editors : _4d0,
		finder : {
			getTr : function(_564, _565, type, _566) {
				type = type || "body";
				_566 = _566 || 0;
				...
				if (_566 == 0) {
					...
					if (type == "body") {
						return (_566 == 1 ? dc.body1 : dc.body2)
								.find(">table>tbody>tr[datagrid-row-index="
										+ _565 + "]");
					} else {
						if (type == "footer") {
							return (_566 == 1 ? dc.footer1 : dc.footer2)
									.find(">table>tbody>tr[datagrid-row-index="
											+ _565 + "]");
						} else {
							if (type == "selected") {
...
(function($) {
	function _585(_586) {
		...
		$(_586).datagrid($.extend({}, opts, {
					view : (opts.showGroup ? _587 : undefined),
					onClickRow : function(_588, row) {
						if (opts.editIndex != _588) {
							...
							$(this).datagrid("getEditors", _588)[0].target
									.focus();
							opts.editIndex = _588;
						}
		...
		$(_586).datagrid("getPanel").find("div.datagrid-body")
	...
	$.fn.propertygrid.parseOptions = function(_58f) {
		var t = $(_58f);
		return $.extend({}, $.fn.datagrid.parseOptions(_58f), {
					showGroup : (t.attr("showGroup")
							? t.attr("showGroup") == "true"
							: undefined)
				});
	};
	var _587 = $.extend({}, $.fn.datagrid.defaults.view, {
		render : function(_590, _591, _592) {
			...
			for (var i = 0; i < _596.length; i++) {
				var _597 = _596[i];
				_594.push("<div class=\"datagrid-group\" group-index=" + i
						+ " style=\"\">");
				_594
				...
				if (!_592) {
					...
					_594.push(opts.groupFormatter.call(_590, _597.fvalue,
							_597.rows));
					_594.push("</span>");
				}
				_594.push("</td>");
				...
				for (var j = 0; j < _597.rows.length; j++) {
					var cls = (_595 % 2 && opts.striped)
							? "class=\"datagrid-row-alt\""
							: "";
					var _598 = opts.rowStyler ? opts.rowStyler.call(_590, _595,
							_597.rows[j]) : "";
					var _599 = _598 ? "style=\"" + _598 + "\"" : "";
					_594.push("<tr datagrid-row-index=\"" + _595 + "\" " + cls
							+ " " + _599 + ">");
					_594.push(this.renderRow.call(this, _590, _593, _592, _595,
							_597.rows[j]));
					_594.push("</tr>");
					_595++;
				}
			...
			view.find("div.datagrid-group").each(function() {
				...
				$(this).find("div.datagrid-row-expander").bind("click", {
							...
							if ($(this).hasClass("datagrid-row-collapse")) {
								$(_59a).datagrid("collapseGroup",
										e.data.groupIndex);
							} else {
								$(_59a).datagrid("expandGroup",
										e.data.groupIndex);
							}
						});
			});
				...
				expandGroup : function(jq, _5a6) {
					return jq.each(function() {
								...
								if (_5a8.hasClass("datagrid-row-expand")) {
									_5a8.removeClass("datagrid-row-expand")
				...
				collapseGroup : function(jq, _5a9) {
					return jq.each(function() {
								...
								if (_5ab.hasClass("datagrid-row-collapse")) {
									_5ab.removeClass("datagrid-row-collapse")
...
(function($) {
	...
	function _5b0(_5b1) {
		...
		if (opts.pagination) {
			...
			_5b9.pagination({
						...
						pageSize : opts.pageSize,
						pageList : opts.pageList,
						onSelectPage : function(_5ba, _5bb) {
	...
	function _5ce(_5cf) {
		...
		_5d1.find("input[type=checkbox]").unbind().bind("click.treegrid",
				function() {
					if (opts.singleSelect) {
						return false;
					}
	...
	function _5d4(_5d5, _5d6) {
		...
		var tr2 = _5d8.children("div.datagrid-body").find("tr[node-id=" + _5d6
				+ "]");
		var _5d9 = $(_5d5).datagrid("getColumnFields", true).length
				+ (opts.rownumbers ? 1 : 0);
		...
		function _5db(tr, _5dc) {
			$("<tr class=\"treegrid-tr-tree\">"
					+ "<td style=\"border:0px\" colspan=\"" + _5dc + "\">"
	...
	function _5dd(_5de, _5df, data, _5e0) {
		...
		if (node) {
			var _5e3 = _5e1.children("div.datagrid-body").find("tr[node-id="
					+ _5df + "]");
			var _5e4 = _5e2.children("div.datagrid-body").find("tr[node-id="
					+ _5df + "]");
			var cc1 = _5e3.next("tr.treegrid-tr-tree").children("td")
					.children("div");
			var cc2 = _5e4.next("tr.treegrid-tr-tree").children("td")
					.children("div");
		} else {
			var cc1 = _5e1.children("div.datagrid-body")
					.children("div.datagrid-body-inner");
		...
		if (opts.showFooter) {
			opts.view.renderFooter.call(opts.view, _5de, _5e1
							.find("div.datagrid-footer-inner"), true);
			opts.view.renderFooter.call(opts.view, _5de, _5e2
	...
	function _5f8(_5f9, _5fa) {
		...
		var body = $(_5f9).datagrid("getPanel")
	...
	function _606(_607, _608) {
		...
		var node = view.find("div.datagrid-body tr[node-id=" + _608 + "]")
	...
	$.fn.treegrid.methods = {
		...
		reloadFooter : function(jq, _638) {
			return jq.each(function() {
						...
						if (opts.showFooter) {
							opts.view.renderFooter.call(opts.view, this, _639
											.find("div.datagrid-footer-inner"),
									true);
							opts.view.renderFooter.call(opts.view, this, _63a
											.find("div.datagrid-footer-inner"),
									false);
							if (opts.view.onAfterRender) {
								opts.view.onAfterRender.call(opts.view, this);
	...
	$.fn.treegrid.parseOptions = function(_63c) {
		var t = $(_63c);
		return $.extend({}, $.fn.datagrid.parseOptions(_63c), {
	...
	var _63d = $.extend({}, $.fn.datagrid.defaults.view, {
		render : function(_63e, _63f, _640) {
			...
			function _643(_644, _645, _646) {
				...
				for (var i = 0; i < _646.length; i++) {
					...
					var _648 = opts.rowStyler
							? opts.rowStyler.call(_63e, row)
							: "";
					var _649 = _648 ? "style=\"" + _648 + "\"" : "";
					_647.push("<tr node-id=" + row[opts.idField] + " " + _649
							+ ">");
					_647 = _647.concat(view.renderRow.call(view, _63e, _641,
							_644, _645, row));
					_647.push("</tr>");
		...
		renderRow : function(_64f, _650, _651, _652, row) {
			...
			for (var i = 0; i < _650.length; i++) {
				...
				if (col) {
					var _654 = col.styler
							? (col.styler(row[_653], row) || "")
							: "";
					var _655 = col.hidden ? "style=\"display:none;" + _654
							+ "\"" : (_654 ? "style=\"" + _654 + "\"" : "");
		...
		refreshRow : function(_656, id) {
			...
			tr.children("td").each(function() {
				...
				if (col) {
					var _65a = col.styler
							? (col.styler(row[_659], row) || "")
							: "";
					var _65b = col.hidden ? "display:none;" + _65a : (_65a
							? _65a
							: "");
		...
		onBeforeRender : function(_65c, _65d, data) {
			...
			if (node) {
				...
				$.data(_65c, "treegrid").data = $.data(_65c, "treegrid").data
						.concat(data);
			}
			if (!opts.remoteSort) {
				this.sort(_65c, data);
		...
		sort : function(_661, data) {
			...
			function _663(rows) {
				rows.sort(function(r1, r2) {
							return _662(r1[opts.sortName], r2[opts.sortName])
									* (opts.sortOrder == "asc" ? 1 : -1);
						});
				for (var i = 0; i < rows.length; i++) {
	...
	$.fn.treegrid.defaults = $.extend({}, $.fn.datagrid.defaults, {
		...
		singleSelect : true,
		view : _63d,
		loadFilter : function(data, _668) {
			return data;
		},
		finder : {
			getTr : function(_669, id, type, _66a) {
				type = type || "body";
				_66a = _66a || 0;
				...
				if (_66a == 0) {
					...
					if (type == "body") {
						return (_66a == 1 ? dc.body1 : dc.body2)
								.find("tr[node-id=" + id + "]");
					} else {
						if (type == "footer") {
							return (_66a == 1 ? dc.footer1 : dc.footer2)
									.find("tr[node-id=" + id + "]");
						} else {
							if (type == "selected") {
								return (_66a == 1 ? dc.body1 : dc.body2)
...
(function($) {
	function _66e(_66f, _670) {
		...
		_672.panel("resize", {
					width : (opts.panelWidth ? opts.panelWidth : _671
							.outerWidth()),
	...
	function init(_677) {
		...
		var _678 = $("<input type=\"text\" class=\"combo-text\">")
				.appendTo(span);
						...
						if (opts.editable) {
							...
							_67f.timer = setTimeout(function() {
										...
										if (_67f.previousValue != q) {
											...
											_684(_67e);
											opts.keyHandler.query.call(_67e,
													_682.val());
											_68d(_67e, true);
										}
									}, opts.delay);
	...
	function _684(_685) {
		...
		function _689() {
			...
			if (left + _687.outerWidth() > $(window).width()
					+ $(document).scrollLeft()) {
		...
		function _688() {
			...
			if (top + _687.outerHeight() > $(window).height()
					+ $(document).scrollTop()) {
	...
	function _6a1(_6a2, _6a3) {
		...
		for (var i = 0; i < _6a3.length; i++) {
			var _6a6 = $("<input type=\"hidden\" class=\"combo-value\">")
					.appendTo(_6a5);
			if (name) {
				_6a6.attr("name", name);
			}
	...
	$.fn.combo = function(_6af, _6b0) {
		...
		return this.each(function() {
					...
					$("input.combo-text", _6b1.combo).attr("readonly",
							!_6b1.options.editable);
					_674(this);
	...
	$.fn.combo.parseOptions = function(_6b6) {
		var t = $(_6b6);
		return $.extend({}, $.fn.validatebox.parseOptions(_6b6), {
			width : (parseInt(_6b6.style.width) || undefined),
...
(function($) {
	...
	function _6cb(_6d0, _6d1, _6d2) {
		...
		_6d3.find("div.combobox-item-selected")
		...
		for (var i = 0; i < _6d1.length; i++) {
			...
			vv.push(v);
			ss.push(s);
			_6d3.find("div.combobox-item[value=" + v + "]")
	...
	$.fn.combobox.methods = {
		...
		clear : function(jq) {
			return jq.each(function() {
						...
						_6ea.find("div.combobox-item-selected")
								.removeClass("combobox-item-selected");
					});
		},
		loadData : function(jq, data) {
	...
	$.fn.combobox.parseOptions = function(_6ed) {
		var t = $(_6ed);
		return $.extend({}, $.fn.combo.parseOptions(_6ed), {
...
(function($) {
	function _6f1(_6f2) {
		...
		tree.tree($.extend({}, opts, {
					...
					cascadeCheck : (opts.cascadeCheck !== false),//是否级联选取
					onLoadSuccess : function(node, data) {
		...
		(function				() {
					...
					onCheck : function(node, _6f6) {
						//add by xjf@20120309 点击树和复选框时，按ctrl键可以支持级联选取
						var o = tree.tree("options");
						if (o.event && o.event.ctrlKey) {
							o.cascadeCheck = true;
						} else {
							o.cascadeCheck = (opts.cascadeCheck !== false);
						}
						//----add end-----
						//_6f7(_6f2);
					...
					//add by xjf@20120303
					onAfterCheck : function(node, _6a2) {
						_6f7(_6f2);
	...
	function _6f7(_6f8) {
		...
		vv = [vv.join(opts.separator)];//add by xjf@20120301
		$(_6f8).combo("setValues", vv)
	...
	$.fn.combotree.parseOptions = function(_703) {
		return $.extend({}, $.fn.combo.parseOptions(_703), $.fn.tree
						.parseOptions(_703));
	};
...
(function($) {
	...
	function _70f(_710, step) {
		...
		if (_713.length) {
			_712 = grid.datagrid("getRowIndex",
					_713[_713.length - 1][opts.idField]);
			_712 += step;
			if (_712 < 0) {
				_712 = 0;
			}
	...
	$.fn.combogrid.parseOptions = function(_721) {
		var t = $(_721);
		return $.extend({}, $.fn.combo.parseOptions(_721), $.fn.datagrid
						.parseOptions(_721), {
...
(function($) {
	function _722(_723) {
		...
		function _725() {
			...
			_724.calendar = $("<div></div>").appendTo(_726)
					.wrap("<div class=\"datebox-calendar-inner\"></div>");
			_724.calendar.calendar({
						fit : true,
						border : false,
						onSelect : function(date) {
			...
			$("<a href=\"javascript:void(0)\" class=\"datebox-current\"></a>")
					.html(opts.currentText).appendTo(_728);
			$("<a href=\"javascript:void(0)\" class=\"datebox-close\"></a>")
					.html(opts.closeText).appendTo(_728);
...
(function($) {
	function _738(_739) {
		...
		$(_739).datebox("calendar").calendar({
					onSelect : function(date) {
		...
		if (!_73a.spinner) {
			var p = $("<div style=\"padding:2px\"><input style=\"width:80px\"></div>")
					.insertAfter(_73c.children("div.datebox-calendar-inner"));
			_73a.spinner = p.children("input");
			_73a.spinner.timespinner({
						showSeconds : true
					}).bind("mousedown", function(e) {
						e.stopPropagation();
					});
			...
			var ok = $("<a href=\"javascript:void(0)\" class=\"datebox-ok\"></a>")
					.html(opts.okText).appendTo(_73d);
			ok.hover(function() {
						$(this).addClass("datebox-button-hover");
					}, function() {
						$(this).removeClass("datebox-button-hover");
	...
	function _743(_745, _746, _747) {
		...
		function _748(date) {
			...
			return tt.join($(_745).datetimebox("spinner")
